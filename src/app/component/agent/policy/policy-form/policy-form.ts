...
export class PolicyForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<Agent>(`${this.server}/agents/me`).subscribe(agent => {
      this.currentAgent = agent;
      this.form.patchValue({ agentCode: agent.agentCode});
    });
  ...
  activate(): void {
    ...
    this.http.patch(`${this.server}/policies/${this.savedPolicyNumber()}/activate`, {})
      .subscribe({
        next: () => {
          this.router.navigate(['/agent/policies', this.savedPolicyNumber()]);
        },
